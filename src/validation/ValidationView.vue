<template>
    <div class="container">
        ...
        <div class="row mb-4">
            <div class="col">
                <h1 class="text-center">Question-Answering on Temporal Relations (Answer Validation)</h1>
            </div>
        </div>
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="card-body">
                                ...
                                <div class="card-text">
                                    <span style='color:DodgerBlue;'>[WARNING] </span><strong>We know the answer to some of these event candidates. If we see unreasonably low performance on these candidates, we will reject your hits.</strong>
                                </div>
                                <div class="card-text">
                                    <span style='color:DodgerBlue;'>[TIP] </span><strong>Be careful of negative, hypothetical, and uncertain events. Feel free to check our guidelines <a target="_blank" href="https://qatmr-qualification.github.io/?tab=training-qa">here</a> when you're not sure about certain rules. Please do pay attention to the rules on "Future Events".</strong>
                                </div>
                                <div class="card-text">
                                    <span style='color:DodgerBlue;'>[TIP] </span><strong>In addition, "what (has) happened?" and "what happened before something A?" require the answer to have finished before now or before A; in contrast, "what will happen" and "what happened after something" does NOT have this requirement.</strong>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                          ...
                          Passage {{staging.passage_id + 1}} / {{number_of_passages}}
                                            ...
                                            <button
                                                    ...
                                                    class="btn btn-secondary"
                                                    style="float: right"
                                                    v-on:click="next_passage"><i class="fas fa-chevron-right"></i></button>
                                        </h4>
                                    </div>
                    ...
                    && staging.current_question.text!==''">
                          ...
                          <span id="eventSelIns"><small>Double-click on single words (words connected by "-" are single) as events in the passage. <span style="color:Tomato">Only verbs or nouns, including "be" verbs, are qualified, but not all of them are events</span>; see <a href="https://qatmr-qualification.github.io/?tab=training-event" target="_blank">event instructions</a>.</small>
                            ...
                            'list-group-item-success': questions_created_satisfied,
                                            ...
                                            >
                                                Questions answered [required]: <span style="float: right">{{current.question_created}} / {{this.required_question_count}}</span>
                                            </li>
<!--                                            <li class="list-group-item"-->
<!--                                                v-bind:class="{-->
                            ...
                            <div class="col-lg-8" v-if="this.staging.showing_qa_panel">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div
                                            v-if="staging.current_question.question_id !== null"
                                        >
                                            <div
                                                    class="row"
                                            >
...
<!--                                                    >-->
                                                    ...
                                                    <div style="text-decoration: underline; font-size: large; color: darkblue">
                                                        {{staging.current_question.text}}
                                                    </div>
                                            ...
                                            <div class="row mt-4">
                                                <div class="col">
                                                    <h4>*All* valid answers from the event list</h4>
                                                    <div
                                                            class="input-group mb-3"
                                                    >
                                                        <input
...
<script>
    ...
    export default {
        ...
        mounted: async function(){
            ...
            let load_events = "load_events" in parsed_query_string ? parsed_query_string.load_events === "1" : true;
            this.allow_modify_event = "allow_modify_event" in parsed_query_string ? parsed_query_string.allow_modify_event === "1" : false;
            const load_answers = "load_answers" in parsed_query_string ? parsed_query_string.load_answers === "1" : true;
            ...
            _.each(validation_multiple_tasks, (validation_task) =>{
                console.log(validation_task);
                if (validation_task.worker_id === workerId){
                    // Same work, need to instruct worker to return this hit.
                    this.fatal_error_msg = "This hit contains annotations that were previously provided by yourself. Since you shouldn't verify your own annotations, please skip this hit and move onto the next one.";
                    return;
                }
                _.each(validation_task.passages, (passage) => {
                    ...
                    _.each(passage.question_answer_pairs, (question) => {
                        ...
                        if (load_answers){
                            _.each(question.answer.indices, (start_end_str) => {
                                const str_pair = start_end_str.substring(1, start_end_str.length-1).split(",")
                                const start = parseInt(str_pair[0]);
                                const end = parseInt(str_pair[1]) - 1;
                                const key = start + "_" + end;
            ...
            this.seconds_remain = "freeze_time" in parsed_query_string ? parseInt(parsed_query_string.freeze_time) : 180;
        ...
        methods: {
            ...
            can_add_answer: function(event){
                ...
                if (this.staging.current_question.text.length === 0){
                    this.show_qa_error("You have to select a question first.");
                    return false;
                }
